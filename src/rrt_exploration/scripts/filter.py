...
def callBack(data, args):
    ...
    transformedPoint = tf_listener.transformPoint(target_frame, data)
...
def globalMap(data):
    ...
    if n_robots > 1:
        ...
        # In Python 3 this may be bytes -> decode it
        if isinstance(topic, bytes):
            topic = topic.decode()
        indx = int(topic[litraIndx]) - namespace_init_count
    elif n_robots == 1:
        indx = 0
...
def node():
    ...
    global globalmaps, litraIndx, n_robots, namespace_init_count
    ...
    # wait if any of robots' global costmap is not received yet
    for i in range(0, n_robots):
